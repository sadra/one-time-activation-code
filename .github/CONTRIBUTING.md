# Contributing
we are happy that you want to contribute to this project, first of all we know it is a bit painful but please it is important to read the [code of conduct file](https://github.com/sadra/one-time-activation-code/blob/master/.github/CODE_OF_CONDUCT.md).
...
* `npm run test:watch` - Test and watch all the Typescript/Javascript files in the `lib` folder with jest.
* `npm run lint` - Lint check style and watch all the Typescript/Javascript files in the `lib` folder with eslint and prettier.
* `npm run format` - To format your codes.
...
* Format your codes - `npm run format`.
* Build ts files - `npm run build`
